<div class='kiwi-map-container'>
  ...
  if (isset($_POST['google-key'])) {
      if (get_option('kiwi_google_reviews_api_key')) {
          update_option('kiwi_google_reviews_api_key', $_POST['google-key']);
      } else {
          add_option('kiwi_google_reviews_api_key', $_POST['google-key'], '', 'yes');
      }
  }

  if (! empty($_POST['place_id'])) {
      echo '<h2 style="text-align:center">Your Shortcode : <input class="kiwi-google-reviews-input" type="text" value="[kiwi-google-review place-id='.$_POST['place_id'].']" /><button class="kiwi-reviews-btn" onclick="copy()" data-clipboard-text="[kiwi-google-review place-id='.$_POST['place_id'].']"><svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="copy" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="svg-inline--fa fa-copy fa-w-14 fa-2x"><path fill="currentColor" d="M320 448v40c0 13.255-10.745 24-24 24H24c-13.255 0-24-10.745-24-24V120c0-13.255 10.745-24 24-24h72v296c0 30.879 25.121 56 56 56h168zm0-344V0H152c-13.255 0-24 10.745-24 24v368c0 13.255 10.745 24 24 24h272c13.255 0 24-10.745 24-24V128H344c-13.2 0-24-10.8-24-24zm120.971-31.029L375.029 7.029A24 24 0 0 0 358.059 0H352v96h96v-6.059a24 24 0 0 0-7.029-16.97z" class=""></path></svg></button></h2><hr />';
  }
  require_once 'google-places-api.php';

  wp_enqueue_script('places-script', plugins_url('/views/js/query.js', __FILE__), ['jquery'], '', true);
  wp_enqueue_script('google-maps-places-script', 'https://maps.googleapis.com/maps/api/js?key='. get_option('kiwi_google_reviews_api_key') .'&sensor=false&libraries=places&callback=initialize', ['jquery'], '', true);
  ...
  if (empty(get_option('kiwi_google_reviews_api_key')) || isset($_GET['kiwi-google-reviews-change-key'])):
  ?>
  <?php
  if (!isset($_GET['kiwi-google-reviews-change-key'])):
  ?>
  ...
  <form autocomplete="off" action="" method="POST">
    ...
    if (isset($_GET['kiwi-google-reviews-change-key'])):
    ?>
    <h4>Your current API Key : <?php echo get_option('kiwi_google_reviews_api_key') ?></h4>
  ...
  if (!isset($_GET['kiwi-google-reviews-change-key'])):
  if (!empty(get_option('kiwi_google_reviews_api_key'))):
  if (! empty($_POST['place_id'])) {
      $query = $_POST['place_id'];
      require_once 'google-places-api.php';
      $place = new GooglePlaces;
      $info = $place->get_places($_POST['place_id']);
      add_shortcode('kiwi-google-review', 'kiwi_google_reviews', $_POST['place_id']);
      do_shortcode('[kiwi-google-review place-id='.$_POST['place_id'].']');
  } else {
      $query = '';
  }
  ...
  ?>
    ...
    <form autocomplete="off" action="" method="POST">
        <a href="?page=google-reviews&kiwi-google-reviews-change-key=true">Change API key</a>
